...
public class ManagePartTime extends JFrame implements ActionListener{
	
	JPanel panel;
	JLabel accountNoLabel,jobNameLabel,jobStatusLabel,jobLocationLabel,salaryLabel,preferredGenderLabel,ageLimitLabel,shiftLabel;
	...
	public ManagePartTime(ManageJob mj,PartTimeList PartTimes){
		super("MANAGE parttime JOB");
		this.mj = mj;
...
/* 		  	preferredGenderField = new JTextField();
		...
		//======================//
		 ...
		 //======================//
			...
			shiftLabel.setBounds(40,300,260,30);
			panel.add(shiftLabel );
			
			shiftField = new JTextField();
			shiftField.setFont(font);
			shiftField.setBounds(310,300,300,30);
	...
	public void  initialization(){
		...
		JLabel background = new JLabel(new ImageIcon("./Resource/part.jpg"));
	...
	public void createTable(){
		...
		for(int i=0;i<allparttimes.size();i++){
			...
			model.addRow(new Object[]{parttime.getAccountNo(),parttime.getJobName(),
			parttime.getJobStatus(),parttime.getJobLocation(),
			parttime.getSalary(),parttime.getPreferredGender(),parttime.getageLimit(),parttime.getShift()
			});
		}
		...
		FileIO.writeInFile(allLines,"./Files/PartTimes.txt",false);
	}
...
/* 		if(addBtn == e.getSource()){
			...
			PartTime pt = new PartTime(accountNo,jobName,jobStatus,jobLocation,salary,preferredGender,ageLimit
			,shift
			);	
			PartTimes.insert(pt);
			}
		...
		!salaryField.getText().isEmpty() &&
            ...
            int ageLimit = Integer.parseInt(ageLimitField.getText());
            String shift = shiftField.getText();
           	PartTime pt = new PartTime(accountNo, jobName, jobStatus, jobLocation, salary, preferredGender, ageLimit, shift);
          PartTimes.insert(pt);
        } else {
            JOptionPane.showMessageDialog(this, "Enter All Details", "Warning", JOptionPane.WARNING_MESSAGE);
        }
			...
			if(rows != null){
				...
				for(int i=rows.length-1;i>=0;i--){
					...
					PartTimes.removeByAccount(Integer.parseInt(table.getModel().
													getValueAt(rows[i],0).
													toString() ));
