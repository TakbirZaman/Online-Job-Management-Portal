...
public class ManageFullTime extends JFrame implements ActionListener{
	
	JPanel panel;
	JLabel accountNoLabel,jobNameLabel,jobStatusLabel,jobLocationLabel,salaryLabel,preferredGenderLabel,ageLimitLabel,overTimepayRateLabel;
	JTextField accountNoField,jobNameField,jobStatusField,jobLocationField,salaryField,preferredGenderField,ageLimitField,
	overTimepayRateField;
	...
	public ManageFullTime(ManageJob mj,FullTimeList FullTimes){
		super("MANAGE fulltime JOB");
		this.mj = mj;
		...
		//=================//
			...
			accountNoField.setBounds(310,20,300,30);
		...
		//=================//
			...
			jobNameField.setBounds(310,60,300,30);
		...
		//=================//
			...
			jobStatusField.setBounds(310,100,300,30);
		...
		//======================//
			...
			jobLocationField.setBounds(310,140,300,30);
		...
		//======================//
			...
			salaryField.setBounds(310,180,300,30);
...
/* 			 preferredGenderField = new JTextField();
			...
			preferredGenderField.setBounds(310,220,300,30);
        ...
        preferredGenderComboBox.setBounds(310, 220, 300, 30);
		...
		//======================//
			...
			ageLimitField.setBounds(310,260,300,30);
			panel.add(ageLimitField);
			//======================//
		 //======================//
			overTimepayRateLabel = new JLabel("OVER TIME PAY RATE");
			overTimepayRateLabel.setFont(font);
			overTimepayRateLabel.setForeground(Color.BLACK);;
			overTimepayRateLabel.setBounds(40,300,260,30);
			panel.add(overTimepayRateLabel );
			
			overTimepayRateField = new JTextField();
			overTimepayRateField.setFont(font);
			overTimepayRateField.setBounds(310,300,300,30);
			panel.add(overTimepayRateField);
			//======================//
	...
	public void  initialization(){
		...
		JLabel background = new JLabel(new ImageIcon("./Resource/full.jpg"));
	...
	public void createTable(){
		...
		model.addColumn("OVER TIME PAYRATE");
		...
		for(int i=0;i< allFullTimes.size();i++){
			...
			model.addRow(new Object[]{fulltime.getAccountNo(),fulltime.getJobName(),
			fulltime.getJobStatus(),fulltime.getJobLocation(),
			fulltime.getSalary(),fulltime.getPreferredGender(),fulltime.getageLimit()
			,fulltime.getoverTimepayRate()
			});
		}
		...
		FileIO.writeInFile(allLines,"./Files/FullTimes.txt",false);
	}
...
/* 		if(addBtn == e.getSource()){
			if(!accountNoField.getText().isEmpty() && 
			   ...
			   && !overTimepayRateField.getText().isEmpty()
			   ){
			...
			String overTimepayRate = overTimepayRateField.getText();
			FullTime ft = new FullTime(accountNo,jobName,jobStatus,jobLocation,salary,preferredGender,ageLimit
			,overTimepayRate
			);	
			FullTimes.insert(ft);
			}
        ...
        if (!accountNoField.getText().isEmpty() &&
            ...
            int ageLimit = Integer.parseInt(ageLimitField.getText());
            String overTimepayRate = overTimepayRateField.getText();
           	FullTime ft = new FullTime(accountNo, jobName, jobStatus, jobLocation, salary, preferredGender, ageLimit, overTimepayRate);
         FullTimes.insert(ft);
        } else {
            JOptionPane.showMessageDialog(this, "Enter All Details", "Warning", JOptionPane.WARNING_MESSAGE);
        }		
			...
			if(rows != null){
				...
				for(int i=rows.length-1;i>=0;i--){
					...
					FullTimes.removeByAccount(Integer.parseInt(table.getModel().
													getValueAt(rows[i],0).
													toString() ));
