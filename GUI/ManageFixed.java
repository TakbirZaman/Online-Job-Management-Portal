...
public class ManageFixed extends JFrame implements ActionListener{
	
	JPanel panel;
	JLabel accountNoLabel,jobNameLabel,jobStatusLabel,jobLocationLabel,salaryLabel,preferredGenderLabel,ageLimitLabel,
contactDurationLabel;
	JTextField accountNoField,jobNameField,jobStatusField,jobLocationField,salaryField,preferredGenderField,ageLimitField,contactDurationField;
	...
	public ManageFixed(ManageJob mj,FixedList Fixeds){
		super("MANAGE fixed JOB");
		this.mj = mj;
		this.Fixeds = Fixeds;
		initialization();
...
/* 			 preferredGenderField = new JTextField();
		...
		//======================//
			...
			//======================//
		 //======================//
			contactDurationLabel = new JLabel("CONTACT DURATION");
			contactDurationLabel.setFont(font);
			contactDurationLabel.setForeground(Color.BLACK);;
			contactDurationLabel.setBounds(40,300,260,30);
			panel.add(contactDurationLabel );
			
			contactDurationField = new JTextField();
			contactDurationField.setFont(font);
			contactDurationField.setBounds(310,300,300,30);
			panel.add(contactDurationField);
			//======================//
	...
	public void  initialization(){
		...
		JLabel background = new JLabel(new ImageIcon("./Resource/fixed.jpg"));
	...
	public void createTable(){
		...
		model.addColumn("CONTACT DURATION");
		...
		for(int i=0;i<allfixeds.size();i++){
			...
			model.addRow(new Object[]{fixed.getAccountNo(),fixed.getJobName(),
			fixed.getJobStatus(),fixed.getJobLocation(),
			fixed.getSalary(),fixed.getPreferredGender(),fixed.getageLimit()
			,fixed.getcontactDuration()
			});
		}
		...
		FileIO.writeInFile(allLines,"./Files/Fixeds.txt",false);
	}
...
/* 		if(addBtn == e.getSource()){
			if(!accountNoField.getText().isEmpty() && 
			   ...
			   && !contactDurationField.getText().isEmpty()
			   ){
			...
			String contactDuration = contactDurationField.getText();
			Fixed f = new Fixed(accountNo,jobName,jobStatus,jobLocation,salary,preferredGender,ageLimit
			,contactDuration
			);	
			Fixeds.insert(f);
			}
		...
		!salaryField.getText().isEmpty() &&
            ...
            int ageLimit = Integer.parseInt(ageLimitField.getText());
            String contactDuration = contactDurationField.getText();
           	Fixed f = new Fixed(accountNo, jobName, jobStatus, jobLocation, salary, preferredGender, ageLimit, contactDuration);
         Fixeds.insert(f);
        } else {
            JOptionPane.showMessageDialog(this, "Enter All Details", "Warning", JOptionPane.WARNING_MESSAGE);
        }
			...
			if(rows != null){
				...
				for(int i=rows.length-1;i>=0;i--){
					...
					Fixeds.removeByAccount(Integer.parseInt(table.getModel().
													getValueAt(rows[i],0).
													toString() ));
